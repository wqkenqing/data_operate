package net.data.operate.template;
 ...
 **/
public class HbaseTemplateMR implements Tool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ToolRunner.run(new HbaseTemplateMR(), args);
    }
...
//        }
        ...
        job.setJobName("testtoonline");
        TableMapReduceUtil.initTableMapperJob(FILE_INPUT, scan, HbaseTemplateMRMapper.class, Text.class, Text.class, job);

        // 打包运行必须执行的方法
        job.setJarByClass(HbaseTemplateMR.class);
...
//        job.setMapperClass(HbaseMRMapper.class);
        ...
        // 2.2 指定自定义reduce类
        job.setReducerClass(HbaseTemplateMRReducer.class);
...
class HbaseTemplateMRMapper extends TableMapper<Text, Text> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        try {
            ...
            for (Cell cell : value.rawCells()) {
                ...
                jobj.put("val", val);
                context.write(new Text(rowkey), new Text(val));
            }
        } catch (Exception e) {
...
class HbaseTemplateMRReducer extends TableReducer<Text, Text, NullWritable> {
    ...
    protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        for (Text val : values) {
            put.addColumn("info".getBytes(), "coord".getBytes(), val.toString().getBytes());
